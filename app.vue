<template>
	<div id="app">
		...
		<transition-group tag="div" class="board">
			<div class="tile" v-for="tile in tiles" :id="tile.id" :key="tile.id" :class="[{ legal: defineLegalClasses(tile.position) }, { red: findRow(tile.id - 1) === 0 && colors }, { yellow: findRow(tile.id - 1) === 1 && colors }, { blue: findRow(tile.id - 1) === 2 && colors }, { green: findRow(tile.id - 1) === 3 && colors }]" enter-active-class="opacity" @click="checkLegalMove(tile.position)">
	...
	export default {
		...
		methods: {
			defineLegalClasses: function (position) {
				if (this.emptyTilePosition == position + 4 || this.emptyTilePosition == position + 8 || this.emptyTilePosition == position + 12 || this.emptyTilePosition == position - 4 || this.emptyTilePosition == position - 8 || this.emptyTilePosition == position - 12 || this.emptyTileRow == this.findRow(position)) {
					return true;
				}
				return false;
			},
