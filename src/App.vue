<template>
  <div id="app">
    ...
    <div class="buttons" :class="{showing:ham}">
      ...
      <button class="scrmble" @click="colors = !colors, ham = !ham">Row Colors: {{colors}}</button>
    </div>
  </div>
</template>
    ...
    buildList: function() {
      ...
      for (let index = 0; index <= 15; index++) {
        if (index != this.emptyTilePosition) {
          ...
          this.tiles.push({value:this.emptyTilePostion + 1,position:this.emptyTilePosition,id:"empty"},)
        }
      }
      this.ham = false;
    }
